...
def calculate_subtotal(order):
    ...
    #print('Order subtotal is - ', subtotal)
    ...
    for item in order:
        for name, price in item.items():
            if isinstance(price, float):
                subtotal += price
            else: names.append(price)
    subtotal = round(subtotal,2)
    tax = round((subtotal*0.15),2)
    #print('Order subtotal is - ', subtotal, 'Tax paid is', round((subtotal*0.15),2))

    total = subtotal + tax
    # names = [item for item in order: item.values]
